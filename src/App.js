...
class App extends Component {
  ...
  componentDidMount() {
    this.intervalId = setInterval(this.gameTimer, 1000)
  }
  ...
  onClickThumbnailImage = thumbnailUrl => {
    ...
    const imageObj = imagesList.filter(
      eachObj => eachObj.thumbnailUrl === thumbnailUrl,
    )

    const {imageUrl} = imageObj[0]
  ...
  render() {
    const {isGameRunning, category, score, time, imgUrl} = this.state

    const filteredCategoryThumbnails = imagesList.filter(
            ...
            <div className="first-div">
              ...
              <ul className="tab-elements">
                {tabsList.map(eachObj => (
                  <li key={eachObj.tabId}>
                    <button
                      type="button"
                      className={`tab-button ${
                        category === eachObj.tabId ? 'highlight-text' : ''
                      }`}
                      onClick={() => this.onClickTabItem(eachObj.tabId)}
                    >
                      {eachObj.displayText}
                    </button>
                  </li>
              ...
              <ul className="thumbnail-images">
                {filteredCategoryThumbnails.map(eachObj => (
                  <li key={eachObj.id}>
                    <button
                      type="button"
                      className="image-button"
                      onClick={() =>
                        this.onClickThumbnailImage(eachObj.thumbnailUrl)
                      }
                    >
                      <img
